	...
	<div class="row">
		<div class="col-8 col-content">
			<div class="tsd-panel tsd-typography">
				...
				<p>Findy is a mobile app that finds goods and services. This document describes
				how to add new web sites to the search from the app.</p>
				<p>To add new web site to the search you have to create <code>source module</code> for
				that site, every site available on Findy have it&#39;s own source module.</p>
				<p>When user searches for something, app builds search <code>profile</code>. Here is an example:</p>
				<pre><code>const profile = {
  category: <span class="hljs-string">'product:apparel:women:dress'</span>,
  keyword: <span class="hljs-string">'black'</span>,
  geolocation: {
    country: <span class="hljs-string">'US'</span>
  }
}</code></pre><p>Profile always have a <code>category</code> and some other properties. Which properties
					are allowed is determined by the category. Refer to meta package to find out
					which fields are allowed for every category. For profile above see
				<a href="/interfaces/meta.productapparelwomendressprofile.html">ProductApparelWomenDressProfile</a></p>
				<p>On search start every available source module receives a search profile.
				It&#39;s source module&#39;s responsibility to determine whether it can respond to given profile.</p>
				<p>For example, if apparel source receives profile with category <code>pet:dog</code>,
				it answers that profile is not supported.</p>
				<p>If profile is supported by the source module, it will make some http
				requests, and process responses into <code>items</code>.</p>
				<p>An example of an item for dress is:</p>
				<pre><code>const <span class="hljs-literal">item</span> = {
  <span class="hljs-string">"attributes"</span>: {
    <span class="hljs-string">"price"</span>: {
      <span class="hljs-string">"min"</span>: <span class="hljs-number">59.99</span>,
      <span class="hljs-string">"max"</span>: <span class="hljs-number">59.99</span>
    },
    <span class="hljs-string">"priceUnit"</span>: <span class="hljs-string">"USD"</span>,
    <span class="hljs-string">"condition"</span>: [
      <span class="hljs-string">"New"</span>
    ],
    <span class="hljs-string">"marketplace"</span>: <span class="hljs-string">"Retail"</span>
  },
  <span class="hljs-string">"id"</span>: <span class="hljs-string">"070078502"</span>,
  <span class="hljs-string">"title"</span>: <span class="hljs-string">"Patterned Wrap-front Dress"</span>,
  <span class="hljs-string">"photoUrl"</span>: <span class="hljs-string">"https://example.com/1.jpg"</span>,
  <span class="hljs-string">"additionalPhotoUrls"</span>: [
    <span class="hljs-string">"https://example.com/2.jpg"</span>, <span class="hljs-string">"https://example.com/3.jpg"</span>
  ],
  <span class="hljs-string">"url"</span>: <span class="hljs-string">"http://example.com/en_us/productpage.070078502.html"</span>,
  <span class="hljs-string">"category"</span>: <span class="hljs-string">"product:apparel:women:dress"</span>,
  <span class="hljs-string">"location"</span>: {
    <span class="hljs-string">"country"</span>: <span class="hljs-string">"US"</span>
  }
}</code></pre><p>Item type for every product is <a href="/interfaces/meta.productitem.html">ProductItem</a></p>
				<p>For more details read <a href="/#source-implementation">Source Implementation</a></p>
				<h3 id="setting-up-developer-environment">Setting up Developer Environment</h3>
				<h4 id="node">Node</h4>
...
<span class="hljs-built_in">npm</span> run dev</code></pre><p>It will compile module in a watch mode and start instruments:</p>
				...
				<pre><code>npm <span class="hljs-keyword">run</span><span class="bash"> create &lt;<span class="hljs-built_in">source</span>-name&gt;</span></code></pre><p>Where <source-name> is the name of the source you are going to work on. One
					word, do not use punctuation. Command will create source folder inside
				<code>src/</code> and put some example code there.</p>
				<p>You can have more then one source in your workspace and work on them at the
				same time.</p>
				<p>Once source module stub is generated, start adapting the code to your
				web site while checking how it works with the Tool.</p>
				...
				<ul>
					...
					<li>parseData.ts - parses response. See
